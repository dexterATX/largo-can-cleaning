...
// 2 reviews per column × repeat 4 = 8 cards per column
  ...
  return (
    <figure
      className={cn(
        ...
        'w-28 will-change-transform',
        'bg-[var(--concrete-gray)]/60'
      )}
    >
      {/* Author at top */}
  ...
  return (
    ...
    >
      ...
      <div className="relative flex h-[350px] sm:h-[450px] w-full flex-row items-center justify-center gap-3 overflow-hidden [perspective:300px]">
        ...
        >
          <Marquee vertical repeat={4} className="[--duration:30s] [--gap:0.75rem] !p-0">
            {firstRow.map((review) => (
              <ReviewCard key={review.name} {...review} />
            ))}
          </Marquee>
          <Marquee vertical repeat={4} className="[--duration:30s] [--gap:0.75rem] !p-0 [&>div]:![animation-direction:reverse] [&>div]:![animation-delay:-5s]">
            {secondRow.map((review) => (
              <ReviewCard key={review.name} {...review} />
            ))}
          </Marquee>
          <Marquee vertical repeat={4} className="[--duration:30s] [--gap:0.75rem] !p-0 [&>div]:![animation-delay:-10s]">
            {thirdRow.map((review) => (
              <ReviewCard key={review.name} {...review} />
            ))}
          </Marquee>
          <Marquee vertical repeat={4} className="[--duration:30s] [--gap:0.75rem] !p-0 [&>div]:![animation-direction:reverse] [&>div]:![animation-delay:-3s]">
